...
const useUrlQueryParams = ({ size = 15, currentPage }: { size?: number, currentPage?: number | undefined }): { urlValue: IStateParams, writeToUrl: TypeWriteToUrl } => {
    ...
    const writeToUrl = React.useCallback(
        ({ name, value, items }: { name: string, value: number | string | undefined, items?: Array<Record<string | number, any>> }) => {
            if (name.endsWith('_id') && items && items.length) {
                const findElement = items.find(e => e.id === value);
                if (findElement) {
                    value = encodeURIComponent(
                        `${value}+title=${
                            findElement.profile ? findElement.profile.last_name + " " + findElement.profile.first_name + " " + findElement.profile.middle_name : findElement.name
                        }`
                    );
                }
            }
